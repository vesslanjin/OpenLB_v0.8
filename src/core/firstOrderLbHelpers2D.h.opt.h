...
struct rlbHelpers<T, descriptors::D2Q9Descriptor> {
    ...
    T rho, const T u[2], const T pi[3], T omega )
  {
    ...
    T d0 = (T)2 * (-v1 - v2 );
    T d1 = (T)1/(T)4 * (v1 + v2 - pi[1]);
    T d2 = (T)1/(T)2 * ((T)2 * v1 - v2);
    T d3 = (T)1/(T)4 * (v1 + v2 - pi[1]);
    T d4 = (T)1/(T)2 * (- v1 + (T)2*v2);
  
    T omga = (T)1-omega;

    cell[0]  = LH::equilibrium(0, rho, u, uSqr) + omga * d0;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq0(pi);
    cell[1]  = LH::equilibrium(1, rho, u, uSqr) + omga * d1;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq1(pi);
    cell[2]  = LH::equilibrium(2, rho, u, uSqr) + omga * d2;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq2(pi);
    cell[3]  = LH::equilibrium(3, rho, u, uSqr) + omga * d3;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq3(pi);
    cell[4]  = LH::equilibrium(4, rho, u, uSqr) + omga * d4;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq4(pi);
    cell[5]  = LH::equilibrium(5, rho, u, uSqr) + omga * d1;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq1(pi);
    cell[6]  = LH::equilibrium(6, rho, u, uSqr) + omga * d2;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq2(pi);
    cell[7]  = LH::equilibrium(7, rho, u, uSqr) + omga * d3;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq3(pi);
    cell[8]  = LH::equilibrium(8, rho, u, uSqr) + omga * d4;
               //((T)1-omega) * neqPiD2Q9::fromPiToFneq4(pi);
    return uSqr;
  }
