...
addInternalVelocityEdge(int x0, int x1, int y0, int y1, int z0, int z1, T omega)
{
  if (!(( x0==x1 && y0==y1 ) ||
        ( x0==x1 && z0==z1 ) ||
        ( y0==y1 && z0==z1 ) )) clout << x0 <<" "<< x1 <<" "<< y0 <<" "<< y1 <<" "<< z0 <<" "<< z1 << std::endl;
...
template<typename T, template<typename U> class Lattice, class BoundaryManager>
BlockStructure3D<T,Lattice>& BoundaryConditionInstantiator3D<T,Lattice,BoundaryManager>::getBlock()
{
  return block;
}

template<typename T, template<typename U> class Lattice, class BoundaryManager>
BlockStructure3D<T,Lattice> const& BoundaryConditionInstantiator3D<T,Lattice,BoundaryManager>::getBlock() const
{
  return block;
}
